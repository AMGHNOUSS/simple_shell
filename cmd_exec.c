...
int cmd_exec(lists_shell *listssh)
{
	...
	exec = is_executable(listssh);
	if (exec == -1)
		return (1);
	if (exec == 0)
	{
		dir = _which(listssh->args[0], listssh->_environ);
		if (check_error_cmd(dir, listssh) == 1)
			return (1);
	}
	...
	if (pd == 0)
	{
		if (exec == 0)
			...
			dir = listssh->args[0];
		execve(dir + exec, listssh->args, listssh->_environ);
	}
	else if (pd < 0)
	{
		perror(listssh->av[0]);
