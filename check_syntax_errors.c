...
void print_syntax_error(lists_shell *listssh, char *input, int i, int bool)
{
	...
	msg2 = ": Syntax error: \"";
	msg3 = "\" unexpected\n";
	counter = aux_itoa(listssh->counter);
	length = _strlen(listssh->av[0]) + _strlen(counter);
...
int check_syntax_error(lists_shell *listssh, char *input)
{
	...
	if (f_char == -1)
	{
		print_syntax_error(listssh, input, begin, 0);
		return (1);
	}
	...
	if (i != 0)
	{
		print_syntax_error(listssh, input, begin + i, 1);
		return (1);
	}
